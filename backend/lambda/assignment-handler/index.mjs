...
const TABLE_NAME = process.env.ASSIGNMENTS_TABLE || "Assignments";
...
function getAuthorizationHeader(event) {
  ...
  return h.Authorization || h.authorization || h.AUTHORIZATION || "";
}
...
async function handleCreate(event, userId) {
  ...
  const platform = body.platform || body.sourcePlatform || "manual";
...
async function handleUserCreate(event) {
  ...
  const email = (body.email && String(body.email).trim()) || tokenClaims?.email || "";
